...
export type ChargeGoalTarget = {
  goalId: string;
  goalName: string;
  defaultAmount?: number;
};

type ChargeGoalSheetProps = {
  visible: boolean;
  uid: string;
  target: ChargeGoalTarget | null;
  defaultPhone?: string;
  defaultOperator?: Operator;
  onClose: () => void;
};

export default function ChargeGoalSheet({
  visible,
  uid,
  target,
  defaultPhone,
  ...
  // Reset the form each time the sheet is opened for a goal.
  useEffect(() => {
    if (!visible) return;
  ...
  }, [visible, target?.goalId, target?.defaultAmount, defaultPhone, defaultOperator]);
  ...
  const canCharge =
    Boolean(target) && Boolean(parsedAmount) && phoneDigits.length >= 9;
  ...
  const handleCharge = async () => {
    ...
    setErrorMessage('');
    setPhase('awaiting');
    setStatusMessage('Sending request to your wallet…');

    try {
      const charge = await chargeGoal(uid, target.goalId, {
        phone: phoneDigits,
        operator,
        amount: parsedAmount,
      });
      ...
      if (charge.status === 'failed') {
        setPhase('failed');
        setStatusMessage(charge.message || 'The charge could not be started.');
        return;
      }

      setStatusMessage('Approve the prompt on your phone to confirm.');
      await pollConfirm(target.goalId, charge.reference);
    } catch (error) {
      if (cancelledRef.current) return;
      setPhase('failed');
      setStatusMessage(
        error instanceof GoalsApiError
          ? error.message
          : 'Something went wrong starting the charge.'
      );
    }
  };

  const pollConfirm = async (goalId: string, reference: string) => {
    ...
    while (!cancelledRef.current && Date.now() < deadline) {
      try {
        ...
        if (result.applied || result.status === 'successful') {
          setAppliedAmount(result.amountApplied ?? result.goal.currentAmount);
          setStatusMessage('');
          setPhase('success');
          return;
        }
        ...
        if (result.status === 'failed') {
          setPhase('failed');
          setStatusMessage('The payment was declined or failed.');
          return;
        }
      } catch (error) {
        if (cancelledRef.current) return;
    ...
    if (!cancelledRef.current) {
      setPhase('failed');
      setStatusMessage(
        'Timed out waiting for approval. If you approved the prompt, your balance will update shortly.'
      );
    }
  };
        ...
        <View style={[styles.sheet, { paddingBottom: Math.max(insets.bottom, 20) }]}>
          ...
          <View style={styles.headerRow}>
            <Text style={styles.title}>
              {phase === 'success' ? 'Saved!' : 'Save to goal'}
            </Text>
          ...
          {phase === 'form' ? (
            <View>
              ...
              <Pressable
                ...
                style={[styles.primaryButton, !canCharge && styles.buttonDisabled]}
              >
                <Text style={styles.primaryButtonText}>Charge wallet</Text>
              </Pressable>
            </View>
          ) : null}
          ...
          {phase === 'success' ? (
            <View style={styles.statusBlock}>
              ...
              <Text style={styles.successText}>
                ZMW {appliedAmount?.toLocaleString()} added to your goal.
              </Text>
